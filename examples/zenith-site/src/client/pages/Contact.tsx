                            ...
                            <div className="flex flex-col items-center gap-8">
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        <span className="text-xs text-gray-500 uppercase tracking-[0.2em] font-mono mb-1 block">Direct Signal Channel</span>
                                        <p className="text-2xl font-bold text-white tracking-tight">{trans('contact.email')}</p>
                                    </div>
                                </a>

                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full max-w-2xl">
                                    <SocialLink icon={<Github />} label={trans('contact.socials.github')} href="https://github.com/gravito-framework" />
                                    <SocialLink icon={<TwitterXIcon />} label={trans('contact.socials.twitter')} href="https://x.com/Gravito_Core" />
                                    <SocialLink icon={<MessageSquare />} label={trans('contact.socials.discord')} href="#" />
                                </div>
                            </div>
                        </div>
                    </motion.div>
...
function SocialLink({ icon, label, href }: { icon: React.ReactNode, label: string, href: string }) {
    return (
        <a href={href} className="flex flex-col items-center gap-4 p-6 rounded-3xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.08] hover:border-white/10 hover:y-[-4px] transition-all text-center group">
            ...
            <span className="text-[10px] text-gray-500 uppercase tracking-widest font-mono font-bold leading-tight">{label}</span>
        </a>
    )
}
