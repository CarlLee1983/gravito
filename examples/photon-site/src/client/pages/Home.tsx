...
import { 
  ...
  Layers, ShieldCheck, Microscope, Database, Workflow, BarChart3, BookOpen
} from 'lucide-react'
  ...
  return (
    <div className="bg-obsidian min-h-screen text-gray-400 font-sans selection:bg-white/10 selection:text-white grid-texture pb-20">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-12 py-8 flex justify-between items-center pointer-events-none">
        <Link href="/" className="text-xl font-black text-white tracking-tighter uppercase pointer-events-auto group">
          Pho<span className="opacity-50 italic group-hover:opacity-100 transition-opacity">ton</span>
        </Link>
        <div className="flex gap-8 pointer-events-auto">
          <Link href="/docs/intro" className="text-[10px] font-black tracking-[0.4em] text-white/50 hover:text-photon-gold transition-colors uppercase">
            Documentation_
          </Link>
          <a href="https://github.com" className="text-[10px] font-black tracking-[0.4em] text-white/50 hover:text-white transition-colors uppercase">
            GitHub_
          </a>
        </div>
      </nav>
      ...
      <section className="relative z-30 max-w-7xl mx-auto px-12 mt-[-100px]">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/5 border border-white/5 backdrop-blur-xl">
          {quickLinks.map((link) => (
            <Link key={link.label} href={link.href} className="p-8 hover:bg-white/[0.02] transition-all group">
              <span className="text-[8px] font-technical text-photon-gold block mb-2 opacity-50 group-hover:opacity-100">{link.desc}</span>
              <span className="text-xs font-black text-white tracking-widest uppercase flex items-center gap-2">
                {link.label} <ArrowUpRight size={12} className="text-gray-700 group-hover:text-photon-gold transition-colors" />
              </span>
            </Link>
          ))}
        </div>
      ...
      <section className="py-20 px-12 max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-32 items-start text-left">
          <div>
            ...
            <Link href="/docs/benchmarks" className="inline-flex items-center gap-6 text-[10px] font-black tracking-[0.5em] text-white uppercase group py-4 px-8 border border-white/5 hover:border-photon-gold/30 transition-all">
      ...
      <section className="py-40 px-12 border-t border-white/5 bg-[#010101]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-24 items-center">
          ...
          <div className="flex-1 w-full glass-card p-12 bg-[#050505] relative group">
            ...
            <h5 className="text-technical text-photon-gold mb-8">// READY_FOR_DEEP_LEARNING</h5>
            <Link href="/docs/intro" className="inline-block px-12 py-6 bg-white text-black font-black text-xs tracking-[0.4em] uppercase hover:bg-photon-gold transition-all relative z-10 shadow-[0_0_50px_rgba(255,255,255,0.1)]">
              Enter Documentation
            </Link>
          </div>
        </div>
