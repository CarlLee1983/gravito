...
export default function Home() {
  ...
  const t = {
    en: {
      ...
      docs_intro: 'Docs_',
      docs_desc: 'Our documentation is more than just a reference. It is a technical deep-dive into zero-copy memory management, AOT compilation, and non-blocking I/O.',
    ...
    'zh-TW': {
      ...
      master: '精通引擎。',
      docs_intro: '文件_',
      docs_desc: '我們的文件不僅僅是參考手冊。它是關於零拷貝記憶體管理、AOT 編譯和非阻塞 I/O 的技術深度探討。',
      ready: '// 深度學習就緒',
    ...
    <div className="min-h-screen font-sans selection:bg-photon-gold/20 transition-colors duration-500 bg-p-bg text-s-txt">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 px-12 py-6 flex justify-between items-center transition-all duration-500 ${scrolled ? 'bg-s-bg/80 backdrop-blur-xl border-b border-s-brd py-4 shadow-2xl' : 'bg-transparent'
        ...
        <div className="flex items-center gap-6">
          <Link href={`/docs/intro?lang=${currentLang}`} className="relative px-6 py-2 group hidden md:block">
      ...
      <div className="grid-texture">
        ...
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-4 animate-bounce opacity-20">
          <span className="text-[8px] font-technical tracking-[0.5em] text-p-txt uppercase">{t.scroll}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-32 items-start text-left">
            <div>
              ...
              <h2 className="text-7xl font-black text-p-txt uppercase tracking-tighter leading-[0.9] mb-12">
                {t.near_zero} <br /> <span className="text-photon-gold italic font-light opacity-80">{t.overhead}</span>
              </h2>
              ...
              <Link href={`/docs/performance?lang=${currentLang}`} className="inline-flex items-center gap-6 text-[10px] font-black tracking-[0.5em] text-p-txt uppercase group py-5 px-10 bg-surf-bg border border-s-brd hover:border-photon-gold hover:text-photon-gold transition-all shadow-xl">
                {t.view_metrics}
        ...
        <section className="py-40 px-12 border-t border-s-brd bg-s-bg">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-24 items-center">
            <div className="flex-1">
              <h3 className="text-6xl font-black text-p-txt uppercase tracking-tighter mb-8 italic opacity-20">{t.docs_intro}</h3>
              <h4 className="text-3xl font-black text-p-txt uppercase tracking-tighter mb-8">{t.master}</h4>
              <p className="text-s-txt mb-12 font-light leading-loose text-lg">
                {t.docs_desc}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Link href={`/docs/routing?lang=${currentLang}`} className="p-6 border border-s-brd bg-surf-bg hover:border-photon-gold/30 transition-all group shadow-sm">
                ...
                <Link href={`/docs/context?lang=${currentLang}`} className="p-6 border border-s-brd bg-surf-bg hover:border-photon-gold/30 transition-all group shadow-sm">
            ...
            <div className="flex-1 w-full glass-card p-12 bg-surf-bg border border-s-brd relative group shadow-2xl">
              ...
              <h5 className="text-technical text-photon-gold mb-8 uppercase tracking-[0.4em]">{t.ready}</h5>
              <Link href={`/docs/intro?lang=${currentLang}`} className="inline-block px-12 py-6 bg-p-txt text-p-bg font-black text-[10px] tracking-[0.4em] uppercase hover:bg-photon-gold hover:text-black transition-all relative z-10 shadow-xl">
                {t.start_reading}
              </Link>
            </div>
          </div>
