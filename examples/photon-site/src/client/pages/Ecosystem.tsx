...
const translations = {
    en: {
        title: 'Ecosystem',
        subtitle: 'Photon is an atomic core. Extend its capabilities using Orbits—plug-and-play modules designed for the Gravito framework.',
        typeLabel: 'Type',
        install: 'Install_Shell',
        buildTitle: 'Build Your Own',
        buildDesc: 'Have a micro-service or utility that benefits the Gravito ecosystem? Join the registry and provide atomic power to thousands of nodes.',
        registryDocs: 'Registry Documentation',
        orbitType: { OFFICIAL: 'OFFICIAL', THIRD_PARTY: 'THIRD_PARTY' }
    },
    'zh-TW': {
        title: '生態系統',
        subtitle: 'Photon 是一個原子核心。使用 Orbits 擴展其功能——專為 Gravito 框架設計的隨插即用模組。',
        typeLabel: '類型',
        install: '安裝 Shell',
        buildTitle: '構建您自己的',
        buildDesc: '擁有對 Gravito 生態系統有益的微服務或工具？加入註冊表，為成千上萬的節點提供原子動力。',
        registryDocs: '註冊表文檔',
    ...
    return [
        {
            ...
            description: isZh ? '先進的身份驗證與授權核心。支援 JWT、OAuth2 和 RBAC。' : 'Advanced authentication and authorization kernel. JWT, OAuth2, and RBAC support.',
            icon: Shield,
            type: 'OFFICIAL',
            features: isZh ? ['JWE 支援', '會話防護', '角色管理'] : ['JWE Support', 'Session Shield', 'Role Management']
        },
        {
            name: 'Atlas',
            package: '@gravito/atlas',
            description: isZh ? 'Photon 的重量級 Data Mapper ORM。針對 PostgreSQL、MySQL、MariaDB 和 SQLite 的高速 SQL 編排。' : 'The heavyweight Data Mapper ORM for Photon. High-speed SQL orchestration for PostgreSQL, MySQL, MariaDB, and SQLite.',
            icon: Database,
            type: 'OFFICIAL',
            features: isZh ? ['AOT 查詢', '多資料庫支援', '實體映射'] : ['AOT Queries', 'Multi-DB Support', 'Entity Mapping']
        },
        {
            name: 'Prism',
            package: '@gravito/prism',
            description: isZh ? '具有 Blade 風格語法和預編譯渲染的原生模板引擎。' : 'Native template engine with Blade-inspired syntax and pre-compiled rendering.',
            icon: Layout,
            type: 'OFFICIAL',
            features: isZh ? ['邏輯區塊', '佈局 Yields', '高速 JIT'] : ['Logic Blocks', 'Layout Yields', 'High-Speed JIT']
        },
        {
            name: 'Ion',
            package: '@gravito/ion',
            description: isZh ? 'Inertia.js 的內部 SPA 橋樑。輕鬆構建現代單體應用。' : 'Internal SPA Bridge for Inertia.js. Build modern monoliths easily.',
            icon: Zap,
            type: 'OFFICIAL',
            features: isZh ? ['共享狀態', '握手協議', '資產版本控制'] : ['Shared State', 'Handshake Protocol', 'Asset Versioning']
        }
    ]
}
