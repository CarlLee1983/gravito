...
const products = [
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1627435601361-ec25f5b1d0e5?q=80&w=1200&auto=format&fit=crop',
    theme: 'text-lime-900',
    bg: 'bg-lime-50',
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1606312619070-d48b706521bf?q=80&w=1200&auto=format&fit=crop',
...
const ProductStage = ({ product, index }: { product: (typeof products)[0]; index: number }) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className="block group">
      <div
        ref={ref}
        className={`min-h-[90vh] flex flex-col md:flex-row items-center relative overflow-hidden ${isEven ? '' : 'md:flex-row-reverse'}`}
      >
        {/* Background Texture */}
        <div
          className={`absolute inset-0 ${product.bg} opacity-30 transition-colors duration-700`}
        />
        ...
        >
          <span
            className={`text-[25vw] font-display opacity-10 select-none ${product.theme} whitespace-nowrap`}
          >
            {product.name}
          </span>
        </motion.div>
        ...
        <div className="w-full md:w-1/2 h-[50vh] md:h-full p-8 md:p-24 relative flex items-center justify-center z-10">
          ...
          >
            ...
            {/* Overlay Gradient */}
            <div
              className={`absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-60 group-hover:opacity-40 transition-opacity duration-500`}
            />
        ...
        <div className="w-full md:w-1/2 p-8 md:p-24 relative z-10 flex flex-col justify-center">
          <motion.div style={{ y }}>
            <span
              className={`font-sans tracking-[0.3em] uppercase text-sm mb-4 block ${product.accent}`}
            >
              Single Origin
            </span>
            ...
            <div className="flex items-center gap-4 text-ink-black group-hover:gap-8 transition-all duration-300">
              <span className="font-sans tracking-[0.2em] uppercase border-b border-ink-black pb-1">
                View Detail
              </span>
