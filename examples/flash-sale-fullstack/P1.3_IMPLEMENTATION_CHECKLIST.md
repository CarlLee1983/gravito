...
- `src/cache/events/priority.ts` (50 行)
...
**預期時間**：3 小時
**交付物**：
- `src/cache/events/EventAggregator.ts` (250 行)
- `src/cache/events/EventQueue.ts` (200 行)
- `src/cache/events/EventDeduplicator.ts` (150 行)
- `src/cache/events/BackpressureManager.ts` (100 行)

**單元測試**：
- [ ] `tests/cache/EventAggregator.test.ts` (30+ 用例)
- [ ] 事件提交延遲 < 1ms
- [ ] 聚合率 > 80%
