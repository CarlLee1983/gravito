...
此專案複雜性較高（64 個核心包 + 15 個 Satellite + Galaxy Architecture），根據任務複雜度選擇不同模型：

**使用指南**：

| 場景 | 使用模型 | 原因 |
|------|--------|------|
| **規劃與架構設計** | **Opus 4.6** | 深度推理、複雜系統分析、跨包依賴設計 |
| **實作新功能、修復 bug、重構代碼** | **Sonnet 4.6** | 代碼品質高、執行快、平衡質量與速度 |
| **簡單任務**（測試執行、文件閱讀、查詢） | **Haiku 4.5** | 成本效益、速度快 |
