...
if (rounded < threshold) {
  console.error(
    `luminosity-adapter-photon coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(
  `luminosity-adapter-photon coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`
)
