...
1. [x] 實作信號量限制 (In-memory Semaphore)
2. [x] 新增刷新超時控制 (refreshTimeout)
3. [x] 新增重試策略 (maxRetries)
4. [x] 新增刷新統計 (getFlexibleStats)
5. [x] 測試並發刷新行為 (`flexible-cache.test.ts`)

---

## 實作總結 (已完成核心優化)

已顯著改善 Flexible 快取在背景刷新時的資源佔用與可監控性：
- **並發控制**：透過信號量機制，確保單個程序內針對同一鍵值的背景刷新任務同時僅有一個在執行。
- **逾時保護**：新增 `refreshTimeout` 配置，防止背景任務因資料來源回應過慢而長時間佔用資源（預設 30s）。
- **運作分析**：提供 `getFlexibleStats()` 介面，可用於監控背景刷新的成功率與平均耗時。
