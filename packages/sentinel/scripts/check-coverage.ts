...
if (rounded < threshold) {
  console.error(
    `sentinel coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`sentinel coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
