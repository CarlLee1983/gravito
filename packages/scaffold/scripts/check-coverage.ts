...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '50')
...
if (rounded < threshold) {
  console.error(
    `scaffold coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`scaffold coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
