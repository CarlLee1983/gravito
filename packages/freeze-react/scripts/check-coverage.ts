...
if (rounded < threshold) {
  console.error(
    `freeze-react coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`freeze-react coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
