...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '62')
...
if (rounded < threshold) {
  console.error(
    `fortify coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`fortify coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
