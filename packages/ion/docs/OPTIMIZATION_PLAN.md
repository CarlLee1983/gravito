...
- [ ] **1.1** 新增 `rootVars` 參數傳遞測試
- [ ] **1.2** 新增 HTTP status codes 測試 (201, 400, 500 等)
- [ ] **1.3** 新增 URL 解析邊界情況測試
- [ ] **1.4** 新增 props 特殊字符處理測試
- [ ] **1.5** 新增 undefined/null props 處理測試
- [ ] **1.6** 新增 Lazy Props（函數型 props）測試
- [ ] **1.7** 新增 HTML 轉義邏輯完整性測試
...
- [ ] **5.1** 實現 shared props 緩存機制
- [ ] **5.2** 優化 HTML 轉義函數性能
- [ ] **5.3** 考慮 props 序列化結果緩存
