...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '100')
...
if (rounded < threshold) {
  console.error(
    `astral coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`astral coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
