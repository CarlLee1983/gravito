...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '74')
...
if (rounded < threshold) {
  console.error(
    `atlas coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`atlas coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
