...
export class PostgresDriver implements DriverContract {
    ...
    async execute(sql: string, bindings: unknown[] = []): Promise<ExecuteResult> {
        ...
        try {
            ...
            if (result.rows && result.rows.length > 0) {
                ...
                insertId = (firstRow['id'] ?? firstRow['ID'] ?? firstRow[Object.keys(firstRow)[0] ?? '']) as number | bigint | undefined
            }
    ...
    private async loadPgModule(): Promise<PgModule> {
        try {
            ...
            throw new Error(
                'PostgreSQL driver requires the "pg" package. Please install it: bun add pg'
            )
        }
    }
}
