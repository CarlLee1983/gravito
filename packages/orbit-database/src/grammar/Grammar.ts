...
export abstract class Grammar implements GrammarContract {
     ...
     */
    protected compileWhereWithOffset(where: WhereClause, offset: number): { sql: string; bindingsUsed: number } {
        switch (where.type) {
            case 'basic':
    ...
    protected countWhereBindings(where: WhereClause): number {
        switch (where.type) {
            case 'basic': return 1
            case 'in': return where.values?.length ?? 0
            case 'between': return 2
            case 'raw': return where.bindings?.length ?? 0
            case 'nested': return where.bindings?.length ?? 0
            default: return 0
        }
    }
     ...
     */
    protected compileHavingWithOffset(having: HavingClause, offset: number): { sql: string; bindingsUsed: number } {
        if (having.type === 'raw') {
    ...
    protected compileOrders(query: CompiledQuery): string {
        const orders = query.orders
            .map((order) => this.compileOrder(order))
            .join(', ')
