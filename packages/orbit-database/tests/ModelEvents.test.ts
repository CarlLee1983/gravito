import { expect, test, describe, beforeEach, spyOn, jest } from 'bun:test'
...
class EventUser extends Model {
    ...
    async onSaving() { this.events.push('saving') }
    async onSaved() { this.events.push('saved') }
    async onCreating() { this.events.push('creating') }
    async onCreated() { this.events.push('created') }
    async onUpdating() { this.events.push('updating') }
    async onUpdated() { this.events.push('updated') }
    async onDeleting() { this.events.push('deleting') }
    async onDeleted() { this.events.push('deleted') }
    async onRetrieved() { this.events.push('retrieved') }
}
