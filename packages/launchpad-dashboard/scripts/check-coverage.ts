...
if (rounded < threshold) {
  console.error(
    `launchpad-dashboard coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(
  `launchpad-dashboard coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`
)
