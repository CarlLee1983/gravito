...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '90')
...
if (rounded < threshold) {
  console.error(
    `echo coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`echo coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
