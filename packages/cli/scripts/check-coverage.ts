...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '40')
...
if (rounded < threshold) {
  console.error(
    `cli coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`cli coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
