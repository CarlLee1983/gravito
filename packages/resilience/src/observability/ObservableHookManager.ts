...
import { HookManager, type HookManagerConfig } from '@gravito/core'
...
export class ObservableHookManager extends HookManager {
  private metricsCollector?: EventMetrics
  private eventTracer?: EventTracer
  private tracingManager?: EventTracing
  ...
  constructor(config: HookManagerConfig = {}, obsConfig: ObservabilityConfig = {}) {
    ...
    if (obsConfig.enabled && obsConfig.metrics) {
      this.metricsCollector = new (EventMetrics as any)(obsConfig.metrics, obsConfig.metricsPrefix)(
        // Set EventMetrics on the event queue so circuit breakers can record metrics
        this.getEventQueue() as any
      ).setEventMetrics(this.metricsCollector)
    }

    // Initialize tracer if enabled
    if (obsConfig.enabled && obsConfig.tracing) {
      this.eventTracer = new (EventTracer as any)()
      this.tracingManager = new (EventTracing as any)()(
        // Set EventTracing on the event queue so it can create listener spans
        this.getEventQueue() as any
      ).setEventTracing(this.tracingManager)
    }
  }
  ...
  async doActionAsync<TArgs = unknown>(
    ...
    if (this.tracingManager) {
      span = this.tracingManager.startDispatchSpan(
        hook,
        callbacks.length,
    ...
    try {
      ...
      if (this.metricsCollector) {
        ...
        this.metricsCollector.recordDispatchLatency(hook, priority, duration)
        this.metricsCollector.recordProcessed(hook, 'success')

        // Update queue depth for this priority
        const queueDepth = this.getQueueDepthByPriority(priority as 'high' | 'normal' | 'low')
        this.metricsCollector.updateQueueDepth(priority, queueDepth)
      }

      // Set span status to OK
      if (span && this.tracingManager) {
        this.tracingManager.endDispatchSpan(span, 'ok')
      ...
      if (this.metricsCollector) {
        ...
        this.metricsCollector.recordFailure(hook, errorName)
        this.metricsCollector.recordProcessed(hook, 'failure')
      }

      // Record error in span
      if (span && this.tracingManager && error instanceof Error) {
        this.tracingManager.endDispatchSpan(span, 'error', error)
  ...
  async doActionSync<TArgs = unknown>(hook: string, args: TArgs): Promise<void> {
    ...
    let span: any
    if (this.tracingManager) {
      span = this.tracingManager.startDispatchSpan(hook, callbacks.length, 'normal')
    ...
    if (this.metricsCollector) {
      this.metricsCollector.recordDispatchLatency(hook, 'normal', duration)
      this.metricsCollector.recordProcessed(hook, hasError ? 'failure' : 'success')

      if (hasError && lastError) {
        this.metricsCollector.recordFailure(hook, lastError.constructor.name)
      }
    }
    ...
    if (span && this.tracingManager) {
      if (hasError && lastError) {
        this.tracingManager.endDispatchSpan(span, 'error', lastError)
      } else {
        this.tracingManager.endDispatchSpan(span, 'ok')
      }
    } else if (span && this.eventTracer) {
  ...
  setObservabilityConfig(config: Partial<ObservabilityConfig>): void {
    ...
    if (this.obsConfig.enabled && this.obsConfig.metrics && !this.metricsCollector) {
      this.metricsCollector = new (EventMetrics as any)(
        this.obsConfig.metrics,
        this.obsConfig.metricsPrefix
      )(
        // Set EventMetrics on the event queue so circuit breakers can record metrics
        this.getEventQueue() as any
      ).setEventMetrics(this.metricsCollector)
