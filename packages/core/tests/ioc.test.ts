...
import { PlanetCore, type GravitoOrbit, defineConfig } from '../src';
...
describe('PlanetCore IoC', () => {
    ...
    it('should load orbits defined in config (Class Reference)', async () => {
        ...
        expect(core.config.get('mock-orbit:loaded')).toBe(true);
    });
    ...
    it('should pass config to orbits', async () => {
        ...
        expect(core.config.get('mock-orbit:value')).toBe('foobar');
    });
