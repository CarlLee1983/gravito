...
describe('OTelEventMetrics', () => {
  ...
  describe('setQueueDepthCallback', () => {
    it('should set queue depth callback', () => {
      const metrics = new OTelEventMetrics(mockMeter)
      const callback = () => ({
        high: 10,
        normal: 50,
      ...
      metrics.setQueueDepthCallback(callback)

      expect(mockObservableGauge.addCallback).toHaveBeenCalled()
    })

    it('should call the provided callback when gauge is observed', () => {
      let observerCallback: ((result: { observe: ReturnType<typeof mock> }) => void) | undefined

      // Set up mock to capture callback BEFORE creating the metrics instance
      const localMockObservableGauge = {
        addCallback: mock((cb: (result: { observe: ReturnType<typeof mock> }) => void) => {
          observerCallback = cb
        }),
        removeCallback: mock(() => {
          // noop - mock implementation
      ...
      const localMockMeter = {
        createHistogram: mock(() => mockHistogram),
        createObservableGauge: mock(() => localMockObservableGauge),
      }

      const metrics = new OTelEventMetrics(localMockMeter as unknown as MockMeter)
      ...
      metrics.setQueueDepthCallback(queueCallback)

      // Simulate observer calling the callback
      ...
      if (!observerCallback) {
        throw new Error('Observer callback was not set')
      }
      observerCallback(mockObserverResult)

      expect(queueCallback).toHaveBeenCalled()
      expect(mockObserverResult.observe).toHaveBeenCalledWith(5, { priority: 'high' })
      expect(mockObserverResult.observe).toHaveBeenCalledWith(20, { priority: 'normal' })
      expect(mockObserverResult.observe).toHaveBeenCalledWith(35, { priority: 'low' })
    })

    it('should handle zero queue depths', () => {
      let observerCallback: ((result: { observe: ReturnType<typeof mock> }) => void) | undefined
      ...
      expect(mockObserverResult.observe).toHaveBeenCalledWith(0, { priority: 'high' })
      expect(mockObserverResult.observe).toHaveBeenCalledWith(0, { priority: 'normal' })
      expect(mockObserverResult.observe).toHaveBeenCalledWith(0, { priority: 'low' })
    })
    ...
    it('should handle large queue depths', () => {
      ...
      expect(mockObserverResult.observe).toHaveBeenCalledWith(1_000_000, { priority: 'high' })
    })
  })
