 ...
 *
 * ESM-only 實現：使用 dynamic import() 代替 require()
 * 避免 Bun 生成 CJS compatibility code
 */
...
export class NativeAccelerator {
   ...
   * 2. 檢查 Bun 運行時可用性
   * 3. 使用 dynamic import() 載入 bun:ffi 的 cc()
   */
  static async isAvailableAsync(): Promise<boolean> {
    ...
    try {
      ...
      // 使用 dynamic import 而非 require()
      // 這避免了 Bun 生成 CJS compatibility code
  ...
  static isAvailable(): boolean {
    ...
    // 第一次檢查時發起異步檢查，但立即返回 false
    // 這樣不會阻塞應用啟動
    this.isAvailableAsync().catch(() => {})
    return false
  }
   ...
   */
  static async getCborAcceleratorAsync(): Promise<CborAccelerator> {
    ...
    if (await this.isAvailableAsync()) {
      try {
        const native = await this.loadNativeImplementationAsync()
        if (native !== null) {
  ...
  static getCborAccelerator(): CborAccelerator {
    ...
    // 若無緩存，使用 JavaScript fallback
    // 異步載入會在背景進行
    const fallback = new CborFallbackEncoder()
    this.cborAccelerator = fallback
    this.updateStatus('js-fallback')

    // 同時發起異步檢查，嘗試改用 native
    this.getCborAcceleratorAsync().catch(() => {})
  ...
  /**
   * 載入原生 C 實現（異步版本）
   * 使用 dynamic import() 避免 CJS compatibility code
   */
  private static async loadNativeImplementationAsync(): Promise<CborAccelerator | null> {
    try {
      // 檢查 Bun 運行時
      if (typeof Bun === 'undefined') {
        return null
