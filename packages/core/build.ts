...
const pkgName = basename(import.meta.dirname) // "core"
...
const externalDeps = ['@gravito/photon', 'bun:test', 'bun:sqlite', 'zod']
...
async function buildInParallel() {
  ...
  if (!isDtsOnly) {
    ...
    // Task 1: bun build ESM（主入口 + compat + engine 子入口）
    const esmPromise = (async () => {
      const buildResult = await build({
        entrypoints: ['src/index.ts', 'src/compat.ts', 'src/engine/index.ts'],
        outdir: 'dist',
        format: 'esm',
        target: 'node',
        splitting: false,
    ...
    // Task 2: bun build CJS（主入口 + compat）
