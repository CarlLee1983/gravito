...
if (rounded < threshold) {
  console.error(
    `luminosity-adapter-express coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(
  `luminosity-adapter-express coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`
)
