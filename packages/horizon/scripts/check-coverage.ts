...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '60')
...
if (rounded < threshold) {
  console.error(
    `horizon coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`horizon coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
