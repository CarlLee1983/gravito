...
const threshold = Number.parseFloat(process.env.COVERAGE_THRESHOLD ?? '55')
...
if (rounded < threshold) {
  console.error(
    `stream coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`stream coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
