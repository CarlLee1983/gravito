...
if (rounded < threshold) {
  console.error(
    `radiance coverage ${rounded}% is below threshold ${threshold}%. Covered lines: ${hit}/${total}.`
  )
  process.exit(1)
}

console.log(`radiance coverage ${rounded}% (${hit}/${total}) meets threshold ${threshold}%.`)
