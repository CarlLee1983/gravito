...
import { FluxEngine, createWorkflow, MemoryStorage } from '../src'
...
const uploadWorkflow = createWorkflow('image-processing')
    ...
    .step('resize', async (ctx) => {
        ...
        ctx.data.thumbnail = await imageProcessor.resize(
            ctx.input.fileBuffer,
            200,
            200
        )
    })
    .step('compress', async (ctx) => {
        ...
        ctx.data.compressed = await imageProcessor.compress(
            ctx.input.fileBuffer,
            80
        )
    })
