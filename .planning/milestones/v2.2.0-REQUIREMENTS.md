# Requirements Archive: v2.2.0 Framework Evolution
...
- [x] **DX-01**: Route registration retains Zod input/output schemas as metadata accessible to downstream consumers (OpenAPI generator, documentation tools)
- [x] **DX-02**: Developer can generate static OpenAPI 3.1 spec via `gravito openapi:generate` CLI command, emitting `openapi.json` as build-time artifact
- [x] **DX-03**: Developer can define Lite Satellite as object literal in `gravito.config.ts` with single `install(core)` hook, without requiring full directory structure or ServiceProvider class
...
- [x] **TOOL-01**: Developer can generate application-level Orbit/Satellite dependency graph via CLI command showing module coupling relationships
...
| DX-01 | Phase 30 | Complete |
| DX-02 | Phase 30 | Complete |
| DX-03 | Phase 29 | Complete |
...
*Last updated: 2026-03-31 after Phase 32 audit trail completion — all 10 requirements satisfied*
