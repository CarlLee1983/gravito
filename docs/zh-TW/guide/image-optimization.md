...
# 🖼️ 圖片優化 (Image Optimization)

Gravito 內建了強大的**圖片優化**模組（屬於 `Orbit-View` 的一部分）。它參考了 Next.js 的最佳實踐，確保您的應用程式符合最高性能標準，但其核心優勢在於：**零客戶端運行路徑 (Zero Client-side Runtime)**。

---

## 🚀 為什麼要使用 Gravito Image？

不同於標準的 `<img>` 標籤，Gravito 的 `Image` 元件會自動處理複雜的性能優化：

- **防止佈局抖動 (CLS Prevention)**：自動設置 `width` 與 `height`，確保瀏覽器在圖片下載完成前就預留空間。
- **響應式圖片 (Responsive Images)**：根據您的設定自動生成 `srcset` 與 `sizes` 屬性。
- **延遲載入 (Lazy Loading)**：預設為 `loading="lazy"`，僅對標記為關鍵的圖片立即載入。
- **現代性能標準**：預設使用 `decoding="async"` 並支援 `fetchpriority` 來優化 LCP 指標。
- **零 JS 腳本**：所有邏輯都在服務端執行，不會向瀏覽器發送沉重的 JavaScript 套件。

---

## 🛠️ 在 React (Inertia) 中使用
  ...
  return (
    <section>
      ...
      <Image 
        ...
        height={600}
        loading="eager"      // 立即加載
        fetchpriority="high" // 告知瀏覽器這是最高優先級
      />
...
如果您使用我們內建的模板引擎，請使用 `image` 助手（Helper）。
...
2.  **路徑邏輯**：如果您使用 `srcset`，Gravito 預設您的構建流程會生成帶有寬度後綴的圖片（如 `hero-800w.jpg`）。如果您使用外部圖片服務（如 Cloudinary），可以自定義 `ImageService` 的路徑邏輯。
