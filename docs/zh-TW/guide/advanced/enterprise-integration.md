...
1.0.0-beta 聚焦於 Bun 原生效能與一致的企業架構路線。Gravito 強調明確的模組邊界、可預期的依賴關係，以及可審計的執行流程。
...
Gravito 鼓勵乾淨的分層與明確的責任界線，讓商業規則在基礎設施變動時仍保持穩定。

- **領域優先建模**：以聚合、Value Object、Domain Service 描述業務意圖。
- **Use Case 邊界**：Command / Query 分離，降低耦合並提升可測性。
- **依賴外圍化**：DB、Queue、Mail 等基礎設施放在邊界，避免污染核心領域。
- **服務化拆分**：當規模擴大，可將模組獨立成服務而不重寫核心流程。
