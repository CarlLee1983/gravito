...
export class Money extends ValueObject<MoneyProps> {
  ...
  static of(amount: number, currency = 'TWD'): Money {
    if (amount < 0) {
      throw FlashSaleError.invalidAmount(amount)
    }
  ...
  multiply(factor: number): Money {
    if (factor < 0) {
      throw new FlashSaleError(
        `Invalid multiply factor: ${factor}`,
        FlashSaleErrorCode.INVALID_AMOUNT,
        400
      )
    }
    return new Money({
  ...
  private assertSameCurrency(other: Money): void {
    if (this.props.currency !== other.props.currency) {
      throw FlashSaleError.currencyMismatch(this.props.currency, other.props.currency)
    }
  }
}
